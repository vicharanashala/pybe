...
import { getScenarios, getSessions, getAnalytics, getRoadmap, submitSession } from '../api/client';

/**
 * The original PyBe V0 experience: submit reasoning about a scenario and get
 * back abstraction mapping, generated code, prompt feedback, and mastery
 * signals. Unchanged in behavior from the previous single-file app, just
 * relocated out of main.jsx and onto its own tab alongside the new Phase 1
 * Scenario Browser.
 */
function ReasoningStudio() {
  ...
  async function submitReasoning(event) {
    event.preventDefault();
    if (!selected || !form.reasoning.trim()) return;
    setSubmitting(true);
    try {
      const result = await submitSession({ ...form, scenarioId: selected._id });
      ...
      <section className="workspace">
        ...
        <div className="main-grid">
          <section className="panel learning-panel">
            ...
            <form onSubmit={submitReasoning} className="learning-form">
              <label>
                Your reasoning
                <textarea
                  required
...
export default ReasoningStudio;
