...
import { useHintEngine } from './hooks/useHintEngine.js';
import { inspectExecution } from './utils/runtimeInspector.js';
import HintEngine from './assistant/HintEngine.jsx';
import VariableInspector from './assistant/VariableInspector.jsx';
...
function TokenButton({ token, onClick, used }) {
  return (
    <button
...
export default function Stage3CodeBuild({
  caseStudy,
  onComplete,
  recordAnalyticsAttempt,
  recordAnalyticsHint,
  recordAnalyticsBlank
}) {
  const { stage3, id: caseStudyId, concepts } = caseStudy;
  const { codeTemplate, tokens } = stage3;
  const conceptName = concepts?.[0] || 'General';
  ...
  // Progressive Hint Hook
  const { attempts, recordIncorrectAttempt, getHintForToken } = useHintEngine(caseStudyId, currentBlankIndex);

  const activeCorrectToken = correctTokensInOrder[currentBlankIndex];
  const activeHintObj = activeCorrectToken ? getHintForToken(activeCorrectToken) : null;
  ...
  const handleTokenClick = (token) => {
    ...
    if (token.value === correctToken?.value) {
      if (recordAnalyticsBlank) {
        recordAnalyticsBlank(attempts === 0, conceptName);
      }
      if (recordAnalyticsAttempt) {
        recordAnalyticsAttempt(true, conceptName);
      }
      setFilledValues((prev) => ({ ...prev, [currentBlankIndex]: token.value }));
    } else {
      if (attempts === 0 && recordAnalyticsHint) {
        recordAnalyticsHint(conceptName);
      }
      recordIncorrectAttempt();
      if (recordAnalyticsAttempt) {
        recordAnalyticsAttempt(false, conceptName);
      }
    }
  };
  ...
  const handleRun = async () => {
    ...
    const result = await inspectExecution(pyodide, assembled);

    setRunOutput({ output: result.output, error: result.error });
    setExecutionDetails({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', animation: 'csFadeIn .4s ease-out' }}>
      ...
      {!isFreeForm && !allBlanksFilled && (
        <HintEngine hintObj={activeHintObj} attempts={attempts} />
      )}
      ...
      {runOutput && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <TerminalOutput output={runOutput.output} error={runOutput.error} />
          {executionDetails && (
            <VariableInspector
              variables={executionDetails.variables}
              trace={executionDetails.trace}
