...
function LevelCard({ level, unlocked, completed, onClick }) {
  ...
  return (
    <div
      ...
      style={{
        background: completed ? C.accentBg : unlocked ? C.cardBg : '#0a0f12',
        border: completed
          ? `1px solid ${C.success}`
          : unlocked
          ? `1px solid ${C.border}`
      ...
      onMouseEnter={(e) => { if (unlocked) { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = '0 8px 28px rgba(0, 243, 255, 0.2)'; } }}
    ...
    >
      ...
      <span style={{ fontSize: '0.72rem', fontWeight: 700, color: C.success, letterSpacing: '0.12em', textTransform: 'uppercase' }}>
        Level {lvl}
      </span>
