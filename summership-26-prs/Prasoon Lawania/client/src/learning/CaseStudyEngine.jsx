...
import { useLearningAnalytics } from './hooks/useLearningAnalytics.js';
import LearningAnalytics from './assistant/LearningAnalytics.jsx';
...
function LevelCompleteScreen({ onBack, onNext, summaryObj }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem', padding: '1rem 2rem', textAlign: 'center', animation: 'csFadeIn .5s ease-out' }}>
      <div style={{ fontSize: '3rem', margin: 0 }}>🏆 Level Complete!</div>
      
      {/* Learning summary component */}
      <LearningAnalytics summaryObj={summaryObj} />

      <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', justifyContent: 'center', marginTop: '0.5rem' }}>
        <button
          onClick={onBack}
          style={{ background: C.cardBg, border: `1px solid ${C.border}`, borderRadius: 10, color: C.body, padding: '0.7rem 1.4rem', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer' }}
        >
          ← Back to Levels
        </button>
        {onNext && (
          <button
            onClick={onNext}
            style={{ background: C.darkBg, border: 'none', borderRadius: 10, color: C.darkText, padding: '0.7rem 1.6rem', fontSize: '0.9rem', fontWeight: 600, cursor: 'pointer' }}
          >
            Next Level →
          </button>
        )}
...
export default function CaseStudyEngine({ levelData, topicLevelCount, levelId, onBack, onGoToLevel }) {
  ...
  const analytics = useLearningAnalytics(levelId);
  ...
  if (levelDone) {
    return (
      <LevelCompleteScreen
        ...
        summaryObj={analytics.getSummary()}
      />
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
      ...
      {currentStage === 3 && (
        <Stage3CodeBuild
          ...
          recordAnalyticsAttempt={analytics.recordAttempt}
          recordAnalyticsHint={analytics.recordHintUsed}
          recordAnalyticsBlank={analytics.recordBlankResolution}
        />
      )}
    </div>
  );
