...
const { enrichScenarioSummary, enrichScenarioDetail } = require('../services/scenarioEnrichment');
...
router.get('/', async (req, res, next) => {
  try {
    ...
    res.json(scenarios.map(enrichScenarioSummary));
  } catch (error) {
    next(error);
  }
});
...
router.get('/:id', async (req, res, next) => {
  try {
    ...
    res.json(enrichScenarioDetail(scenario));
  } catch (error) {
    next(error);
  }
});
