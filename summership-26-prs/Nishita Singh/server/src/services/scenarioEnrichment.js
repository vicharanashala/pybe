...
function buildGuidedQuestions(scenario) {
  // Scenarios generated by the AI scenario generator (services/
  // scenarioGenerationService.js) come with their own guidedQuestions,
  // written specifically for that story's character and details
  // (Enhancement Proposal #2: "guided reasoning must be specific to the
  // scenario, not generic"). Hand-authored scenarios from db.json don't
  // have that field, so they keep using the original generic templates -
  // this is purely additive, nothing about the existing behavior changes.
  if (Array.isArray(scenario.guidedQuestions) && scenario.guidedQuestions.length) {
    return scenario.guidedQuestions.map((entry, index) => ({
