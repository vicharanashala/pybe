...
async function listSessions() {
  ...
  return db.sessions
    .map((session) => ({
      ...
      debugChallenge: db.debugChallenges ? (db.debugChallenges.find((c) => c._id === session.debugChallenge) || null) : null
    }))
...
async function listDebugChallenges(filters = {}) {
  const db = await readDb();
  let challenges = db.debugChallenges || [];
  if (filters.difficulty) challenges = challenges.filter((item) => item.difficulty === filters.difficulty);
  if (filters.concept) challenges = challenges.filter((item) => item.concepts.includes(filters.concept));
  return challenges;
}

async function getDebugChallenge(id) {
  const db = await readDb();
  const challenges = db.debugChallenges || [];
  return challenges.find((challenge) => challenge._id === id) || null;
}

async function resetData(scenarios, debugChallenges = []) {
  await writeDb({
    scenarios: scenarios.map((scenario) => createRecord(scenario)),
    debugChallenges: debugChallenges.map((challenge) => createRecord(challenge)),
    sessions: []
  });
}
