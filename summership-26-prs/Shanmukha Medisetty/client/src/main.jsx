      ...
      <section className="workspace">
            ...
            <form onSubmit={submitSession} className="learning-form">
              ...
              <label>
                ...
                <textarea
                  ...
                  onChange={(event) => setForm({ ...form, learnerCode: event.target.value })}
                  placeholder={'# Try writing your reasoning as Python\n# e.g. bag_weight = 4.5'}
                  spellCheck={false}
                />
              </label>
              <button type="button" className="ghost" onClick={checkCode} disabled={checking || !form.learnerCode.trim()}>
                <Code2 size={17} />{checking ? 'Checking...' : 'Check My Code'}
              </button>
              {liveReview && <CodeReview review={liveReview} title="Live code check" />}
              <label>
                Reflection
                <textarea
        ...
        <section className="dashboard">
          ...
          <div className="panel">
            ...
            <SessionList sessions={sessions} onSelectSession={(session) => setActiveResult(session)} />
          </div>
        </section>
      </section>
    </main>
...
function CodeReview({ review, title }) {
  if (!review?.attempted) return null;
  return (
    <div className="code-review">
      <header>
        <strong>{title}</strong>
        <span className="code-score">{review.score}<small>/100</small></span>
      </header>
      {review.constructsFound.length > 0 && (
        <ul className="construct-list found">
          {review.constructsFound.map((item) => <li key={item}><CheckCircle2 size={15} /> {item}</li>)}
        </ul>
      )}
      {review.constructsMissing.length > 0 && (
        <ul className="construct-list missing">
          {review.constructsMissing.map((item) => <li key={item}><AlertTriangle size={15} /> missing: {item}</li>)}
        </ul>
      )}
      {review.syntaxWarnings.length > 0 && (
        <div className="note">
          <strong>Syntax watch</strong>
          {review.syntaxWarnings.map((item) => <p key={item}>{item}</p>)}
        </div>
      )}
    ...
    <div className="result-stack">
      ...
      {result.learnerCode && (
        <div className="code-block">
          <div><Code2 size={18} /> Your Python Attempt</div>
          <pre>{result.learnerCode}</pre>
        </div>
      )}
      <div className="code-block">
      ...
      <CodeReview review={result.codeReview} title="Your code, reviewed" />
    ...
    <div className="sessions">
      {sessions.length ? sessions.slice(0, 6).map((session) => (
        <article
          ...
          title="Click to view session result"
        >
          <Play size={16} />
          <div>
