...
**Left Section:**
- Render text: *"Understanding Iteration Through a Case Study"*
...
- The row must contain the Module Title and a "Start" button.
...
│                      │                                          │
│   Understanding      │                                          │
│   Iteration Through  │   Module 1: Understanding       [Start]  │
│   a Case Study       │   Iteration                              │
│                      │                                          │
└──────────────────────┴──────────────────────────────────────────┘
...
- **Next Button:** Increments `currentBeat`. Replaced by a "Finish Module" button on the final beat. (Must be disabled if the current beat has a `<z-mcq>` that has not yet been answered correctly).
- **Finish Module Button:** Rendered only on the final beat, in place of Next. **Action:** sets `activeModule` to `null` AND resets `currentBeat` to `1` (so the module restarts from Beat 1 the next time it's opened). This returns the learner to the Landing Page.
