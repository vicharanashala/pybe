...
This document defines the exact structure of the `.md` content files (e.g. `test-module-1.md`, `test-module-2.md`). Parse content files according to this grammar exactly — do not infer structure from examples alone.
...
- **If `type` is `"text"`:** the object has a `content` field, containing an HTML string wrapped in backticks (`` ` ``), not double quotes. Render the HTML inside `content` directly into the Left Section, applying the Design System's tag mapping rules (e.g. translating `style:"..."` notation into Tailwind classes, per `design-system-mapping.md` Section 1).
...
- Copy the text content inside these tags verbatim into the rendered output — per the Copy Accuracy rule in `agent-instructions.md`, do not rewrite or summarize any of it.
...
- `#` vs `##` on the Beat Header (see Section 2).
- Inconsistent indentation inside `left-pane { ... }` or inside code blocks within `right-pane`.

Do NOT silently tolerate:
- A Beat Header with a different `<M>` value than the rest of the file — flag this back to the user rather than guessing which is correct.
- A `left-pane` block missing both `src` and `content` — flag rather than rendering an empty box silently.
