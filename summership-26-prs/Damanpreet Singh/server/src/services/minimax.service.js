...
/**
 * Generates a scenario (case study) using the MiniMax API based on a prompt or topic.
 * @param {string} topic - Topic or title prompt (e.g. "E-commerce Shopping Cart", "Data Analysis", "Game Inventory")
 * @param {string} difficulty - "beginner" | "intermediate" | "advanced"
 */
async function generateScenarioWithMiniMax(topic = 'Python Basics', difficulty = 'intermediate') {
  const apiKey = process.env.MINNIX_API_KEY || process.env.MINIMAX_API_KEY;

  if (!apiKey) {
    throw new Error('MINNIX_API_KEY is not configured in server/.env');
  }
...
Return ONLY valid JSON matching this schema exactly (do not wrap in markdown or extra text):
{
...
3. Make each page lengthy, descriptive, and immersive so the student has a complete, multi-page book reading experience!`;
  ...
  // Single endpoint: Samagama MiniMax proxy
  const endpoint = 'https://samagama.in/platform/proxy/v1/chat/completions';
  ...
  if (!res.ok) {
    ...
    if (res.status === 429) {
      throw new Error('MiniMax rate limit reached. Please wait a moment and try again.');
    }
    throw new Error(`MiniMax API error ${res.status}: ${errBody}`);
  }

  const responseData = await res.json();

  // Parse MiniMax response content
  ...
  // Clean reasoning tags and parse JSON with simple safety recovery
  let cleanJson = content.replace(/<think>[\s\S]*?<\/think>/g, '').trim();
  ...
  try {
    ...
    if (!parsed.context || typeof parsed.context !== 'string' || parsed.context.length < 200) {
      throw new Error(`Generated scenario context too short or incomplete (${parsed.context ? parsed.context.length : 0} characters)`);
    }
  } catch (parseErr) {
    logger.warn(`[MiniMax Scenario Parse Error] ${parseErr.message} - throwing for Gemini fallback`);
    throw new Error(`MiniMax generated malformed or truncated scenario: ${parseErr.message}`);
  }
  ...
  return {
    ...
    context: parsed.context || `Case study on ${topic}`,
...
/**
 * Generates a concept tree (mind map) using MiniMax API.
 * Uses multiple small sequential prompts to avoid rate limits:
 *  - Call 1: root node + 4 branch titles (tiny, ~300 tokens)
 *  - Calls 2-5: one per branch to get 3 leaf children each (~200 tokens)
 * Total: 5 small calls spaced 2.5s apart ≈ ~20s total, well within limits.
 */
async function generateConceptTreeWithMiniMax(topic = 'Loops') {
  const apiKey = process.env.MINNIX_API_KEY || process.env.MINIMAX_API_KEY;
  ...
  /**
   * Parse AI response text into a JSON object, stripping <think> tags and fences.
   */
  ...
  /**
   * Make a single minimal MiniMax API call with auto-retry on 429.
   */
  async function miniMaxCall(messages, retries = 2) {
    for (let attempt = 0; attempt <= retries; attempt++) {
      ...
      if (!res.ok) {
        ...
        if (res.status === 429 && attempt < retries) {
          logger.warn(`MiniMax 429 rate limit, waiting 8s before retry ${attempt + 1}…`);
          await delay(8000);
          continue;
        }
        if (res.status === 429) {
          throw new Error('MiniMax rate limit reached. Please wait a minute and try again.');
        }
        throw new Error(`MiniMax API error ${res.status}: ${errBody}`);
      }
  ...
  // ── STEP 2-N: Expand each branch with leaf children (one call per branch) ───
  for (let i = 0; i < root.children.length; i++) {
    const branch = root.children[i];
    logger.info(`[ConceptTree] Step ${i + 2}/${root.children.length + 1} — expanding "${branch.title}"`);

    await delay(2500); // spread calls across time to avoid rate limits

    try {
      const leafResult = await miniMaxCall([
        {
          role: 'system',
          content: 'You are an engineering system architect. Return ONLY valid JSON. No markdown.\nSchema: {"children":[{"title":"string","desc":"string"}]}'
        },
        {
          role: 'user',
          content: `Give exactly 3 specific child node tools under "${branch.title}" (topic: "${topic}").\nCRITICAL DIAGRAM RULES:\n1) Titles MUST be short ALL-CAPS KEYWORDS (2-3 words max, e.g., "VELOCITY SPIKE", "IP MISMATCH", "PAYLOAD CHECK").\n2) Descriptions MUST be crisp 3-5 word bullet summaries (e.g., "Flag sudden velocity spikes.", "Detect mismatched IP locations."). NO PARAGRAPHS!\nJSON only.`
        }
      ]);
      ...
      logger.warn(`[ConceptTree] Branch "${branch.title}" expansion failed: ${err.message} — skipping`);
      // Partial tree is still useful — continue without this branch's children
    }
  }

  const totalLeaves = root.children.reduce((s, b) => s + b.children.length, 0);
  logger.info(`[ConceptTree] Complete — ${root.children.length} branches, ${totalLeaves} leaves`);
  return { title: topic, root };
}

/**
 * Generates a concept map (nodes + labeled edges + phases) from a case study scenario.
 * Uses 2 small sequential MiniMax calls to avoid rate limits:
 *   Call 1: Extract phases + concept nodes from the scenario
 *   Call 2: Generate labeled relationship edges between concepts
 */
async function generateConceptMapWithMiniMax(scenario) {
  const apiKey = process.env.MINNIX_API_KEY || process.env.MINIMAX_API_KEY;
  if (!apiKey) throw new Error('MINNIX_API_KEY is not configured in server/.env');

  const endpoint = 'https://samagama.in/platform/proxy/v1/chat/completions';
  const delay = (ms) => new Promise((r) => setTimeout(r, ms));

  function safeParseAIJson(raw, fallbackObj = null) {
    try {
      let clean = (raw || '').replace(/<think>[\s\S]*?<\/think>/g, '').trim();
      ...
      logger.warn(`[MiniMax JSON Parser] Recovery fallback invoked: ${err.message}`);
      if (fallbackObj) return fallbackObj;
      throw new Error(`Invalid JSON returned from AI engine: ${err.message}`);
    }
  }

  async function miniMaxCall(messages, fallback = null, retries = 2) {
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        const res = await fetch(endpoint, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
          body: JSON.stringify({ model: 'MiniMax-M3', messages, temperature: 0.5, max_tokens: 2048 }),
        });
        if (!res.ok) {
          const errBody = await res.text();
          if (res.status === 429 && attempt < retries) { await delay(3000); continue; }
          // If MiniMax fails with 403 (No active reservation) or 500, throw error so ai.service.js instantly switches to Gemini 3.6 Flash!
          throw new Error(`MiniMax API error ${res.status}: ${errBody}`);
        }
        const data = await res.json();
        return safeParseAIJson(data.choices?.[0]?.message?.content || '', fallback);
      } catch (err) {
        if (attempt === retries) {
          logger.warn(`[MiniMax Call Failed] Passing error upstream for fallback to Gemini: ${err.message}`);
          if (fallback && err.message.includes('JSON')) return fallback;
          throw err;
        }
        await delay(2000);
      }
...
3. ZERO TOUGH ACADEMIC JARGON: NEVER use confusing college-level engineering words like 'deterministic', 'anomalies', 'encapsulation', 'instantiate', 'override', or 'parameterized'. Instead, use friendly kid-centric words like 'magic locker', 'labeled drawers', 'superpowered loop', 'smart rule guard', 'backpack of items', and 'secret code' so a school child learns effortlessly and feels like a coding hero!`;
  ...
  logger.info(`[MiniMax ConceptMap] Completed multi-step visual story dashboard cleanly!`);
  return {
...
/**
 * Generates a beginner-friendly explanation (analogy, visual points, pseudo-code) for any concept card.
 */
async function explainConceptWithMiniMax(topic, context = '') {
  const apiKey = process.env.MINNIX_API_KEY || process.env.MINIMAX_API_KEY;
  if (!apiKey) throw new Error('MINNIX_API_KEY is not configured in server/.env');
...
async function generateSkeletonWithMiniMax(scenario) {
  const apiKey = process.env.MINNIX_API_KEY || process.env.MINIMAX_API_KEY;
  if (!apiKey) throw new Error('MINNIX_API_KEY is not configured in server/.env');
...
module.exports = { generateScenarioWithMiniMax, generateConceptTreeWithMiniMax, generateConceptMapWithMiniMax, explainConceptWithMiniMax, generateSkeletonWithMiniMax };
